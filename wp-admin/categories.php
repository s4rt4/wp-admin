    ...
    if (!empty($name)) {
        ...
        if ($id > 0) {
            ...
            if ($stmt->execute()) {
                ...
                echo "<div class='notice notice-error is-dismissible'><p>Error updating category: " . $conn->error . "</p></div>";
            }
        } else {
            // Insert
            ...
            if ($stmt->execute()) {
                ...
                echo "<div class='notice notice-error is-dismissible'><p>Error adding category: " . $conn->error . "</p></div>";
            }
        }
    }
}
...
if (isset($_GET['action']) && $_GET['action'] == 'edit' && isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $result_edit = $conn->query("SELECT * FROM categories WHERE id = $id");
...
<div id="wpcontent">
    <div class="wrap">
        ...
        <div id="col-container" style="display: flex; gap: 20px; margin-top: 20px;">
            ...
            <!-- Left Column: Add/Edit Category -->
            <div id="col-left" style="width: 35%;">
                <div class="form-wrap">
                    <h2><?php echo $edit_category ? 'Edit Category' : 'Add New Category'; ?></h2>
                    <form id="addtag" method="post" action="categories.php" class="validate">
                        <?php if ($edit_category): ?>
                            <input type="hidden" name="tag-id" value="<?php echo $edit_category['id']; ?>">
                        <?php endif; ?>
                        
                        <div class="form-field form-required term-name-wrap">
                            <label for="tag-name">Name</label>
                            <input name="tag-name" id="tag-name" type="text" value="<?php echo $edit_category ? htmlspecialchars($edit_category['name']) : ''; ?>" size="40" aria-required="true" required>
                            <p>The name is how it appears on your site.</p>
                        </div>
                        <div class="form-field term-slug-wrap">
                            <label for="tag-slug">Slug</label>
                            <input name="tag-slug" id="tag-slug" type="text" value="<?php echo $edit_category ? htmlspecialchars($edit_category['slug']) : ''; ?>" size="40">
                            <p>The “slug” is the URL-friendly version of the name. It is usually all lowercase and contains only letters, numbers, and hyphens.</p>
                        </div>
                        <div class="form-field term-description-wrap">
                            <label for="tag-description">Description</label>
                            <textarea name="tag-description" id="tag-description" rows="5" cols="40"><?php echo $edit_category ? htmlspecialchars($edit_category['description']) : ''; ?></textarea>
                            <p>The description is not prominent by default; however, some themes may show it.</p>
                        </div>
                        <p class="submit">
                            <input type="submit" name="submit_category" id="submit" class="button button-primary" value="<?php echo $edit_category ? 'Update Category' : 'Add New Category'; ?>">
                            <?php if ($edit_category): ?>
                            ...
                            <?php while($row = $result->fetch_assoc()): ?>
                                <tr class="iedit level-0">
                                    ...
                                    <td class="name column-name has-row-actions column-primary" data-colname="Name">
                                        <strong><a class="row-title" href="categories.php?action=edit&id=<?php echo $row['id']; ?>" aria-label="Edit “<?php echo htmlspecialchars($row['name']); ?>”"><?php echo htmlspecialchars($row['name']); ?></a></strong>
                                        <div class="row-actions">
                                            <span class="edit"><a href="categories.php?action=edit&id=<?php echo $row['id']; ?>" aria-label="Edit “<?php echo htmlspecialchars($row['name']); ?>”">Edit</a> | </span>
                                            <span class="delete"><a href="categories.php?action=delete&id=<?php echo $row['id']; ?>" class="delete-tag aria-button-if-js" aria-label="Delete “<?php echo htmlspecialchars($row['name']); ?>”" role="button" onclick="return confirm('Are you sure?')">Delete</a></span>
                                        </div>
                                    </td>
                                    <td class="description column-description" data-colname="Description"><?php echo htmlspecialchars($row['description']); ?></td>
