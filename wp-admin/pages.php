        ...
        <table class="wp-list-table widefat fixed striped pages">
            <thead>
                <tr>
                    ...
                    <th scope="col" id="builder" class="manage-column column-builder">Builder</th>
                    ...
                    <?php foreach ($pages as $page): ?>
                        <tr class="iedit level-0 page-<?php echo $page['id']; ?> type-page status-<?php echo $page['status']; ?>">
                            ...
                            <td class="status column-status" data-colname="Status">
                                <?php 
                                    $status_label = ucfirst($page['status']);
                                    $status_class = $page['status'] == 'publish' ? 'published' : 'draft';
                                    echo "<span class='post-state $status_class'>$status_label</span>"; 
                                ?>
                            </td>
                            <td class="slug column-slug" data-colname="Slug">
                                <code style="font-size:12px; background:#f0f0f1; padding:2px 6px; border-radius:3px;">/<?php echo htmlspecialchars($page['slug']); ?></code>
                            </td>
                            <td class="date column-date" data-colname="Date">
                                <?php echo $page['status'] == 'publish' ? 'Published' : 'Last Modified'; ?><br>
                                <abbr title="<?php echo $page['updated_at']; ?>"><?php echo date('Y/m/d h:i a', strtotime($page['updated_at'])); ?></abbr>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr class="no-items"><td class="colspanchange" colspan="6">No pages found.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
...
<style>
    ...
    .post-state.draft { color: #996800; }
    .post-state.published { color: #00a32a; }
    ...
    .column-cb { width: 35px; }
    .column-builder { width: 100px; }
    .column-status { width: 80px; }
    .column-slug { width: 180px; }
...
.modal-content {
    ...
    border-radius: 4px; /* WP style doesn't use heavy border radius, but small is fine */
    overflow: hidden;
...
.modal-header h2 {
    margin: 0;
    font-size: 18px; /* Standard WP heading size */
    color: #1d2327;
    font-weight: 600;
}
...
.modal-footer {
    padding: 15px 20px;
    background: #f0f0f1; /* Light grey footer */
...
<script>
    function openQuickEdit(data) {
        ...
        $('#qe-builder_type').val(data.builder);
        
        $('#quick-edit-modal').addClass('show').show(); // .show() for fallback display:none
    }
    ...
    $(document).ready(function() {
        ...
        // Slug generation on title change (optional enhancement)
        $('#qe-post_title').on('input', function() {
            if (!$('#qe-post_name').val()) { // Only auto-fill if slug is empty
